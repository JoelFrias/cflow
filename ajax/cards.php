    ...
    switch ($action) {
        ...
        case 'get_cards':
            ...
            foreach ($cards as $card) {
                ...
                if ($card['credit_limit_usd'] && $card['balance_usd'] > 0) {
                ...
                if ($card['credit_limit_dop'] && $card['balance_dop'] > 0) {
        ...
        case 'add_card_expense':
            ...
            if ($currency === 'USD') {
                ...
                if ($card['credit_limit_usd'] && $new_balance > $card['credit_limit_usd']) {
                ...
                if ($card['credit_limit_dop'] && $new_balance > $card['credit_limit_dop']) {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO transactions
                    (user_id, account_id, category_id, type, amount, original_amount, original_currency, card_currency, converted_amount_dop, date, description)
                VALUES (?, ?, ?, 'expense', ?, ?, ?, ?, ?, ?, ?)
            ");
        ...
        case 'pay_card':
            ...
            $pdo->prepare("UPDATE accounts SET balance = balance - ? WHERE id = ?")->execute([$total_dop_to_debit, $account_id]);
            if ($pay_usd > 0) $pdo->prepare("UPDATE accounts SET balance_usd = balance_usd - ? WHERE id = ?")->execute([$pay_usd, $card_id]);
            if ($pay_dop > 0) $pdo->prepare("UPDATE accounts SET balance_dop = balance_dop - ? WHERE id = ?")->execute([$pay_dop, $card_id]);
        ...
        case 'delete_card':
            ...
            $card = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$card) throw new Exception('Tarjeta no encontrada.');
            if ($card['balance_usd'] > 0 || $card['balance_dop'] > 0) throw new Exception('No puedes eliminar una tarjeta con deuda pendiente.');

            $pdo->prepare("DELETE FROM accounts WHERE id = ? AND user_id = ?")->execute([$card_id, $user_id]);
        ...
        case 'get_card_transactions':
            ...
            /*
            * GASTOS: se guardan con account_id = card_id, type = 'expense',
            *         original_currency = USD|DOP, payment_usd_amount IS NULL
            *
            * PAGOS:  se guardan con account_id = cuenta_bancaria, type = 'expense',
            *         payment_usd_amount > 0 OR payment_dop_amount > 0
            *         No tienen relación directa con card_id en transactions.
            *         Los identificamos buscando en transactions la descripción
            *         que generó pay_card, pero es frágil.
            *
            * Solución correcta: unir ambos conjuntos con UNION.
            */

            $params_expenses  = [$user_id, $card_id];
            $params_payments  = [$user_id, $card_id];

            $where_expenses = '';
            $where_payments = '';

            if ($date_from !== '') {
                $where_expenses .= ' AND t.date >= ?';
                $where_payments .= ' AND t.date >= ?';
                $params_expenses[] = $date_from;
                $params_payments[] = $date_from;
            }
            if ($date_to !== '') {
                $where_expenses .= ' AND t.date <= ?';
                $where_payments .= ' AND t.date <= ?';
                $params_expenses[] = $date_to;
                $params_payments[] = $date_to;
            }
            ...
            // Pagos a la tarjeta (registrados desde la cuenta bancaria)
            // Los relacionamos por card_id guardado en transfer_to_account,
            // o — si no existe ese vínculo — por descripción. 
            // En pay_card usamos transfer_to_account = NULL, así que usamos
            // payment_usd_amount / payment_dop_amount como señal y buscamos
            // todas las transacciones de pago del usuario en el período.
            // Para vincularlos a la tarjeta correcta necesitamos el campo
            // transfer_to_account. Como pay_card no lo usa actualmente,
            // devolveremos los pagos globales del usuario en el período
            // solo cuando no hay filtro de tipo o filtro = 'payment'.
            // Lo mejor es guardar transfer_to_account = card_id en pay_card.
            // Por ahora lo filtramos por descripción como fallback.
            $sql_payments = "
                SELECT
                    t.id,
                    t.date,
                ...
                WHERE t.user_id = ?
                AND t.type    = 'expense'
                AND (t.payment_usd_amount > 0 OR t.payment_dop_amount > 0)
                AND t.description LIKE CONCAT('%', (SELECT creditor FROM debts WHERE id = ? LIMIT 1), '%')
                {$where_payments}
            ";

            // En realidad para tarjetas usamos transfer_to_account.
            // Reemplazamos la query de pagos por la correcta:
            // Pagos son transacciones donde transfer_to_account = card_id
            // PERO pay_card no lo setea. Lo setearemos en el fix de pay_card abajo.
            // Por ahora: pagos = transacciones con (payment_usd_amount > 0 OR payment_dop_amount > 0)
            // del usuario en ese período (no podemos filtrar por tarjeta sin el campo).
            // Usamos la solución correcta: transfer_to_account = card_id.
            ...
            $merge_params = array_merge($params_expenses, $params_payments);
            $sql = '';
            ...
            // Calcular balance acumulado por moneda en orden cronológico inverso
            // (partimos del balance actual y restamos/sumamos hacia atrás)
            ...
            foreach ($transactions as &$t) {
                ...
                if ($t['row_type'] === 'expense') {
                    // El gasto sumó al balance → el balance antes del gasto era menor
                    if ($t['currency'] === 'USD') {
                    ...
                    // El pago restó del balance → el balance antes del pago era mayor
                    $t['balance_after'] = $running_dop;
                    $running_dop = round($running_dop + (float)$t['amount'], 2);
                }
            }
